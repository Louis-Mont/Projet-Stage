...
def insertComm():
    ...
    curGDR.execute(
        "SELECT Commune,CodePostal, Déchèterie, Apport, Domicile FROM Commune WHERE EnrActif = 1 AND (CodePostal = '' or CodePostal != '') ORDER BY CodePostal")
    ...
    for row in CommuneList1:
        ...
        if Commune not in CommTab:
            curSQL.execute(
                "INSERT INTO Commune (Commune, Code_postal, Id_Recyclerie, Id_Insee, Apport, Déchèterie, Domicile) VALUES('%s','%s','%s','%s','%s','%s','%s')" % \
...
def insertArr():
    ...
    curGDR.execute(
        "SELECT to_char(Date,'YYYY/MM/DD'), Origine, Poids_total, IDArrivage FROM Arrivage WHERE IDCommune = 0 AND IDTournée = 0 AND date > %s AND date < %s " % \
        (verifDate, date))
    ArrivList3 = curGDR.fetchall()
    for row in ArrivList3:
        curSQL.execute(
            "INSERT INTO Arrivage (Date, Id_commune, origine, poids_total, Id_recyclerie, Id_tournée) VALUES (?,?,?,?,?,?)",
            (row[0], 0, row[1], row[2], ID_Orga, 0))
    ...
    curGDR.execute(
        "SELECT to_char(Date,'YYYY/MM/DD'), Origine, Poids_total, Tournee.Intitulé, IDArrivage FROM Arrivage, Tournee WHERE IDCommune = 0 AND Tournee.IDTournée = Arrivage.IDTournée AND date > %s AND date < %s " % \
        (verifDate, date))
    ...
    for row in ArrivList2:
        ...
        curSQL.execute(
            "INSERT INTO Arrivage (Date, Id_commune, origine, poids_total, Id_recyclerie, Id_tournée) VALUES (?,?,?,?,?,?)",
            (row[0], 0, row[1], row[2], ID_Orga, ID_tour))
    ...
    curGDR.execute(
        "SELECT to_char(Date,'YYYY/MM/DD'), Origine, Poids_total, Commune.Commune, IDArrivage FROM Arrivage, Commune WHERE Commune.IDCommune = Arrivage.IDCommune AND date > %s AND date < %s " % \
        (verifDate, date))
    ...
    for row in ArrivList:
        ...
        curSQL.execute(
            "INSERT INTO Arrivage (Date, Id_commune, origine, poids_total, Id_recyclerie, Id_tournée) VALUES (?,?,?,?,?,?)",
            (row[0], ID_comm, row[1], row[2], ID_Orga, 0))
...
def InsertProduit(ID_Struc, IDArrivage, Arrivagemax):
    ...
    for row in List:
        ...
        curSQL.execute(
            'INSERT INTO Produit (Orientation, Id_catégorie, Id_Flux, nombre, Id_recyclerie, Poids, Id_arrivage) VALUES (?,?,?,?,?,?,?)',
            (row[3], IDCat, IDFlux, row[0], ID_Struc, row[1], Arrivagemax))
...
def InsertVente():
    ...
    curGDR.execute(
        "SELECT to_char(date,'YYYY/MM/DD'),code_postal,ville,montant_total,tauxremise, IDVente_Magasin from vente_magasin WHERE date > %s AND date < %s ORDER BY code_postal,ville" % \
        (verifDate, date))
    b = curGDR.fetchall()
    for venteorigine in b:
        ...
        curSQL.execute(
            "INSERT INTO Vente (Id_insee, Date, Code_Postal, Commune, Montant_total, TauxRemise, Id_recyclerie) VALUES('%s', '%s','%s','%s','%s','%s','%s') " % \
        ...
        curGDR.execute(
            "SELECT Categorie.Désignation,lignes_vente.montant,lignes_vente.poids,lignes_vente.tauxtva,lignes_vente.montanttva, Flux.Flux, lignes_vente.IDProduit, lignes_vente.IDSous_Catégorie, Sous_Categorie.Désignation FROM lignes_vente, Sous_Categorie, Flux, Categorie WHERE lignes_vente.IDSous_Catégorie = Sous_Categorie.IDSous_Catégorie AND Sous_Categorie.IDFlux = Flux.IDFlux AND Categorie.IDCatégorie = Lignes_Vente.IDCatégorie AND IDVente_Magasin = '%s'" % \
            (venteorigine[5]))
        c = curGDR.fetchall()
        for lignevente in c:
            ...
            curSQL.execute(
                "INSERT INTO Lignes_vente (Id_catégorie,Montant,Poids,Taux_tva,Montant_tva,Id_vente, Id_Flux) values ('%s','%s','%s','%s','%s','%s','%s')" % \
                (IDCat, lignevente[1], poids, lignevente[3], lignevente[4], venteoriginemax, IDFlux))

        curGDR.execute(
            "SELECT Categorie.Désignation,lignes_vente.montant,lignes_vente.poids,lignes_vente.tauxtva,lignes_vente.montanttva,lignes_vente.IDProduit, lignes_vente.IDSous_Catégorie, Sous_Categorie.Désignation FROM lignes_vente, Sous_Categorie, Categorie WHERE Sous_Categorie.IDFlux = 0 AND lignes_vente.IDSous_Catégorie = Sous_Categorie.IDSous_Catégorie AND Categorie.IDCatégorie = Lignes_Vente.IDCatégorie AND IDVente_Magasin = '%s'" % \
            (venteorigine[5]))
        c = curGDR.fetchall()
        for lignevente in c:
        ...
        curGDR.execute(
            "SELECT Categorie.Désignation,lignes_vente.montant,lignes_vente.poids,lignes_vente.tauxtva,lignes_vente.montanttva,lignes_vente.IDProduit FROM lignes_vente, Categorie WHERE Categorie.IDCatégorie = Lignes_Vente.IDCatégorie AND IDSous_Categorie = 0 AND IDVente_Magasin = '%s'" % \
            (venteorigine[5]))
        c = curGDR.fetchall()
        for lignevente in c:
        ...
        curGDR.execute(
            "SELECT Categorie.Désignation,lignes_vente.montant,lignes_vente.poids,lignes_vente.tauxtva,lignes_vente.montanttva,lignes_vente.IDProduit FROM lignes_vente, Categorie WHERE Lignes_Vente.IDSous_Catégorie = 0 and  Categorie.IDCatégorie = Lignes_Vente.IDCatégorie AND IDVente_Magasin = '%s'" % \
            (venteorigine[5]))
        c = curGDR.fetchall()
        for lignevente in c:
...
def souscatJeu(souscat, IDcat):
    ...
    for val in MotCycle:
        if souscat.find(
                val) != -1 and souscat != 'JEUNESSE':  # si la sous_catégorie est identifié dans la liste et n'a pas pour désignation "JEUNESSE" alors on lui attribue la catégorie "JEUX ET JOUETS"
            IDcat = 7
            break
        else:
            continue
...
def correct():
    ...
    if GPS == None:
        ...
        GPS = curSQL.fetchone()
    ID = IDStructure()
    correction_des_communes.correction(connect, curSQL, ID, GPS[0],
                                       GPS[1])  # appel d'une fonction du fichier correction_des_communes
...
try:
    ...
    curSQL.execute("INSERT OR IGNORE INTO Organisation (Recyclerie) VALUES (?) ", (
        RecyclerieNomGDR))  # insère le nom de l'organisation dans la nouvelle base ou non si le nom existe déjà
